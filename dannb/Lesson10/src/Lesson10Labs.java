...
public class Lesson10Labs {
    ...
    static void SecondLab()
    {
        ...
        HashMap<Integer, Boolean> numbersInArray = new HashMap<Integer, Boolean>();
        boolean isDuplicate = false;
        for (int k = 0; k < 10; k++) {        	
        	if (numbersInArray.containsKey(array[k])) {
        		isDuplicate = true;
        	} else {
        		isDuplicate = false;
        	}	
        	numbersInArray.put(array[k], isDuplicate);

        }     

        for (int i = 0; i < 10; i++) {
        	if (numbersInArray.get(array[i]))
                  {
...
//		int previous = fit(i - 1);
//		int twoPrevious = fib(1 - 2);
//		return previous + twoPrevious;
	}
